...
///////////////////////////////////////
{
	const sU8 *d=(const sU8*)a_v2m;//konvertuje ukazatel
	...
	m_base.gptr=d;
  d+=10*m_base.gdnum;
	for (sInt ch=0; ch<16; ch++)//kanaly? 16 midi kanalu
	{
...
//asm("		mov  ecx, this");
/*thisMY=this;
asm("movl _thisMY,%ecx");
//asm("		mov	 eax, [a_time]");
asm("movl 8(%ebp),%eax");
//asm("		mov  ebx, [ecx + m_samplerate]");
PTR=&this->m_samplerate;
asm("movl (_PTR),%ecx");
asm("movl (%ecx),%ebx");//TODO <--?
//asm("		imul ebx");
asm("imul %ebx");
//asm("		mov  ebx, [ecx + m_tpc]");
PTR=&this->m_tpc;
asm("movl (_PTR),%ecx");
asm("movl (%ecx),%ebx");
...
//asm("		idiv ebx");
asm("idiv %ebx");
//asm("		mov  [destsmpl], eax");
destsmplMY=destsmpl;
asm("movl %eax,(_destsmplMY)");*/
destsmpl=(a_time*this->m_samplerate)/this->m_tpc;
...
thisMY=this;
asm("movl _thisMY,%ecx");
//asm("			mov	 eax, [a_fadetime]");
asm("movl 8(%ebp),%eax");
//asm("			mov  ebx, [ecx + m_samplerate]");
m_samplerateMY=m_samplerate;
asm("add _m_samplerateMY,%ecx");
asm("movl (%ecx),%ebx");
asm("sub _m_samplerateMY,%ecx");

//asm("			imul ebx");
asm("imul %ebx");
//asm("			mov  ebx, [ecx + m_tpc]");
m_tpcMY=m_tpc;
asm("add _m_tpcMY,%ecx");
asm("movl (%ecx),%ebx");
asm("sub _m_tpcMY,%ecx");

//asm("			idiv ebx");
asm("idiv %ebx");
//asm("			mov  [ftsmpls], eax");
ftsmplsMY=ftsmpls;
asm("movl %eax,(_ftsmplsMY)");
//		}
